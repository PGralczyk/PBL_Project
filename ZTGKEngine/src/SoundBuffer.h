...
public:
	...
	ALuint getSound(std::string soundName)
	{
		return p_SoundEffectBuffers[soundName];
	}

	ALuint addSoundEffect(const char* filename, std::string soundName)
	{
		ALenum err, format;
		ALuint buffer;
		SNDFILE* sndfile;
		...
		p_SoundEffectBuffers.insert({ soundName, buffer });  // add to the list of known buffers

		return buffer;
	}

	bool removeSoundEffect(std::string soundName)
	{
		auto it = p_SoundEffectBuffers.begin();
		while (it != p_SoundEffectBuffers.end())
		{
